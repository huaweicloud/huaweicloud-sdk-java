...
public class RootVolume implements ModelEntity {
	...
	@java.beans.ConstructorProperties({ "type", "size"})
	public RootVolume(VolumeType type, Integer size) {
		this.type = type;
		this.size = size;
	}
	...
	public RootVolumeBuilder toBuilder() {
		return new RootVolumeBuilder().type(this.type).size(this.size);
	}
	...
	public static class RootVolumeBuilder {
		...
		public RootVolume build() {
			return new RootVolume(type, size);
		}
