...
public class BillService extends BaseBusinessSupportSystemIntlService
{
     ...
     * @return
     */
    public QueryPostpaidBillRsp queryPostpaidBill(String domainID, Map<String, String> filteringParams)
    {
        checkArgument(!Strings.isNullOrEmpty(domainID), "parameter `domainID` should not be empty");

        Invocation<QueryPostpaidBillRsp> queryPostpaidBillInvocation = get(
            QueryPostpaidBillRsp.class, uri("/%s/partner/account-mgr/postpaid-bill-summary", domainID));

        if (filteringParams != null) {
            for (Map.Entry<String, String> entry : filteringParams.entrySet()) {
                queryPostpaidBillInvocation = queryPostpaidBillInvocation.param(entry.getKey(), entry.getValue());
            }
        }
        return queryPostpaidBillInvocation.execute();
    }
